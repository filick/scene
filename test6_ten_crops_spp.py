...
model_conv = load_model(arch, pretrained, use_gpu=use_gpu,AdaptiveAvgPool=AdaptiveAvgPool, SPP=SPP, num_levels=num_levels, pool_type=pool_type)
...
#            outputs = model(inputs)
        ...
        with open(('submit/%s_submit6_%s.json'%(checkpoint_filename, phase)), 'w') as f:
            json.dump(results, f)

        with open(('submit/%s_softmax6_%s.txt'%(checkpoint_filename, phase)), 'wb') as handle:
