...
mlist = []
for _ in label_raw_train:
    mlist.append(_['label_id'])
a = Counter(mlist)
class_sample_count = []
for i in range(0,80):  #注意这里的80，pytorch没有检验这个数字的大小，给1也能跑通，但是输出的都是label=0的图片
...
sampler = torch.utils.data.sampler.WeightedRandomSampler(weights, batch_size)
dataloader = {'train':DataLoader(transformed_dataset_train, batch_size=batch_size,shuffle=False, num_workers=8, sampler=sampler),
             'val':DataLoader(transformed_dataset_val, batch_size=batch_size,shuffle=False, num_workers=8)}
