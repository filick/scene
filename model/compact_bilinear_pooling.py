...
def CountSketchFn_forward(h, s, output_size, x):
    x_size = tuple(x.size())
...
class CountSketchFn(Function):
    ...
    @staticmethod
    def forward(ctx, h, s, output_size, x):
        x_size = tuple(x.size())
...
class CompactBilinearPoolingFn(Function):
    ...
    def backward(ctx,grad_output):
        ...
        return None, None, None, None, None, grad_x, grad_y
...
class CompactBilinearPooling(nn.Module):
    ...
    def forward(self, x):
        x = x.permute(0, 2, 3, 1) #使得输入从NCHW变为NHWC
        y = Variable(x.data.clone())
        out = (CompactBilinearPoolingFn.apply(self.sketch1.h, self.sketch1.s, self.sketch2.h, self.sketch2.s, self.output_size, x, y)).permute(0,3,1,2) #使得输出变为NCHW
